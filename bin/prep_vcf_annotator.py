...
def parse_args(args):
    ...
    parser.add_argument(
        "--vcf", action="store", required=True, help="vcf file from nucdiff"
    )
    parser.add_argument(
        "--outdir", action="store", default=".", help="oudir for reformated file"
    )
    parser.add_argument(
        "--version",
        action="version",
...
def reformat_vcf(file, outdir=".", skip_rows=2):
    ...
    # get the body
    df = pd.read_table(
        file, sep="\t", skiprows=skip_rows, skip_blank_lines=True, index_col=None
    )
    df = df.assign(INFO=".")
